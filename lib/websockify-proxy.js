...
class WebsockifyProxy {
  ...
  setupProxy(ws, target, vmId, connectionId, clientAddr) {
    ...
    const proxyState = {
      isReady: false,
      wsBuffer: [], // Buffer for WebSocket messages before ready
      tcpBuffer: [], // Buffer for TCP data before ready
      totalBufferSize: 0,
      isAlive: true
    };
    ...
    // Set ready state after a short delay to ensure all event handlers are set up
    const readyTimer = setTimeout(() => {
      proxyState.isReady = true;
      log('🟢 Proxy is READY - flushing buffered data');

      // Flush buffered WebSocket messages to TCP
      if (proxyState.wsBuffer.length > 0) {
        log(`   Flushing ${proxyState.wsBuffer.length} buffered WS messages`);
        for (const data of proxyState.wsBuffer) {
          try {
            if (target.writable) {
              const buffer = Buffer.from(data);
              target.write(buffer);
              log(`   📤 [Buffered WS→TCP] Sent ${buffer.length} bytes`);
            }
          } catch (error) {
            log(`   ❌ Error flushing WS buffer: ${error.message}`);
          }
        }
        proxyState.wsBuffer = [];
      }

      // Flush buffered TCP data to WebSocket
      if (proxyState.tcpBuffer.length > 0) {
        log(`   Flushing ${proxyState.tcpBuffer.length} buffered TCP messages`);
        for (const data of proxyState.tcpBuffer) {
          try {
            if (ws.readyState === WebSocket.OPEN) {
              ws.send(data, { binary: true });
              log(`   📥 [Buffered TCP→WS] Sent ${data.length} bytes`);
            }
          } catch (error) {
            log(`   ❌ Error flushing TCP buffer: ${error.message}`);
          }
        }
        proxyState.tcpBuffer = [];
      }

      proxyState.totalBufferSize = 0;
      ...
      if (ws.readyState === WebSocket.OPEN) {
        try {
          ...
          log('✅ Ready notification sent to client');
        } catch (error) {
          log(`⚠️ Failed to send ready notification: ${error.message}`);
        }
      }
    }, 100); // 100ms delay to ensure event handlers are ready

    // Cleanup timer on connection close
    const cleanupTimer = () => {
      if (readyTimer) {
    ...
    ws.on('message', (data) => {
      ...
      console.log(`   Proxy ready: ${proxyState.isReady}`);

      try {
        this.updateActivity(connectionId);

        // If proxy is not ready yet, buffer the data
        if (!proxyState.isReady) {
          const dataSize = data.length || data.byteLength || 0;

          // Check buffer size limit
          if (proxyState.totalBufferSize + dataSize > this.config.bufferMaxSize) {
            log(`⚠️ Buffer limit exceeded, dropping message (${dataSize} bytes)`);
            return;
          }

          proxyState.wsBuffer.push(data);
          proxyState.totalBufferSize += dataSize;
          log(`📦 Buffering WS message (${dataSize} bytes, total: ${proxyState.totalBufferSize} bytes, count: ${proxyState.wsBuffer.length})`);
          return;
        }
    ...
    target.on('data', (data) => {
      try {
        ...
        if (!proxyState.isReady) {
          ...
          // Check buffer size limit
          if (proxyState.totalBufferSize + dataSize > this.config.bufferMaxSize) {
            log(`⚠️ Buffer limit exceeded, dropping TCP data (${dataSize} bytes)`);
            return;
          }

          proxyState.tcpBuffer.push(data);
          proxyState.totalBufferSize += dataSize;
          log(`📦 Buffering TCP data (${dataSize} bytes, total: ${proxyState.totalBufferSize} bytes, count: ${proxyState.tcpBuffer.length})`);
          return;
        }
