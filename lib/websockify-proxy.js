...
class WebsockifyProxy {
  constructor(options = {}) {
    ...
    logger.debug('🚀 WebsockifyProxy initialized');
    logger.debug(`   Max connections per VM: ${this.config.maxConnectionsPerVM}`);
    logger.debug(`   TCP Keepalive: ${this.config.tcpKeepaliveEnable ? 'enabled' : 'disabled'}`);
    logger.debug(`   Buffer max size: ${this.config.bufferMaxSize} bytes`);
    logger.debug(`   Connection retry: max ${this.config.maxRetries} attempts, initial delay ${this.config.retryDelay}ms`);
  }
  ...
  async handleConnection(ws, connectionInfo) {
    ...
    logger.debug(`🔌 New WebSocket connection for VM ${vmId} from ${clientAddr}`);
    logger.debug(`   Connection ID: ${connectionId}`);
    logger.debug(`📡 Target: ${host}:${port}`);

    // Check global max connections limit
    if (this.connections.size >= this.config.maxConnections) {
      logger.error('❌ Global max connections limit reached');
    ...
    if (vmConns.size >= this.config.maxConnectionsPerVM) {
      logger.error(`❌ Max connections limit reached for VM ${vmId} (${vmConns.size}/${this.config.maxConnectionsPerVM})`);
      ws.send(JSON.stringify({
        type: 'error',
    ...
    logger.debug(`🎯 [Early Handler] Registering temporary message buffer for VM ${vmId}`);
    const tempMessageHandler = (data) => {
      if (!isProxyReady) {
        logger.debug(`📦 [Buffer] Buffering message (${data.length || data.byteLength || 0} bytes) while waiting for TCP connection`);
    ...
    try {
      ...
      logger.debug(`✅ [Buffer] TCP connection ready, buffered ${messageBuffer.length} message(s)`);
      ...
      if (messageBuffer.length > 0) {
        logger.debug(`📤 [Buffer] Forwarding ${messageBuffer.length} buffered message(s) to TCP server`);
        for (const data of messageBuffer) {
          const buffer = Buffer.from(data);
          logger.debug(`   → Forwarding ${buffer.length} bytes`);
          if (target.writable) {
            target.write(buffer);
          }
        }
      ...
      logger.debug(`✅ Proxy established for VM ${vmId}`);
      logger.debug(`   Total connections: ${this.connections.size}`);
      logger.debug(`   VM ${vmId} connections: ${vmConnCount}/${this.config.maxConnectionsPerVM}`);

    } catch (error) {
      logger.error(`❌ Failed to establish proxy for VM ${vmId}:`, error.message);

      // Clean up temporary message handler
      if (!isProxyReady) {
        ws.removeListener('message', tempMessageHandler);
        logger.debug(`🧹 [Buffer] Cleaned up temporary message handler (${messageBuffer.length} buffered messages discarded)`);
      }
  ...
  _attemptConnection(host, port, attemptNumber) {
    return new Promise((resolve, reject) => {
      ...
      target.on('connect', () => {
        logger.debug(`✅ TCP connection established to ${host}:${port}${attemptNumber > 1 ? ` (attempt ${attemptNumber})` : ''}`);
        ...
        if (this.config.tcpKeepaliveEnable) {
          ...
          logger.debug(`   TCP Keepalive enabled: initial delay ${this.config.tcpKeepaliveInitialDelay}ms`);
        }

        // Disable Nagle's algorithm for lower latency (important for interactive sessions)
        target.setNoDelay(true);
        logger.debug(`   TCP NoDelay enabled for low latency`);

        resolve(target);
      });

      target.on('error', (error) => {
        logger.error(`❌ TCP connection error to ${host}:${port}:`, error.message);
        reject(error);
      });

      target.on('timeout', () => {
        logger.error(`⏱️ TCP connection timeout to ${host}:${port}`);
        target.destroy();
  ...
  async createTargetConnection(host, port) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (attempt > 1) {
          ...
          logger.debug(`🔄 Retry attempt ${attempt - 1}/${this.config.maxRetries} for ${host}:${port} after ${delay}ms delay`);
        }
        ...
        if (attempt > 1) {
          logger.debug(`✅ Connection successful after ${attempt - 1} retry(ies)`);
        }
        return target;
        ...
        if (attempt < maxAttempts) {
          ...
          logger.debug(`⏳ Connection failed (${error.message}), waiting ${delay}ms before retry...`);
    ...
    // All attempts failed
    logger.error(`❌ All ${maxAttempts} connection attempts failed for ${host}:${port}`);
  ...
  setupProxy(ws, target, vmId, connectionId, clientAddr) {
    const log = (msg) => logger.debug(`[${vmId}/${connectionId}] ${clientAddr}: ${msg}`);

    logger.debug(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
    logger.debug(`🔧 [Proxy Setup] Setting up bidirectional proxy for VM ${vmId}`);
    logger.debug(`   Connection ID: ${connectionId}`);
    logger.debug(`   Client address: ${clientAddr}`);
    logger.debug(`   WebSocket readyState: ${ws.readyState} (1=OPEN)`);
    logger.debug(`   WebSocket protocol: ${ws.protocol || 'none'}`);
    logger.debug(`   TCP socket connected: ${target.writable}`);
    logger.debug(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
    ...
    logger.debug(`📝 [Proxy Setup] Registering 'message' event handler for VM ${vmId}`);
    ws.on('message', (data) => {
      logger.debug(`🎯 [Event] 'message' event fired for VM ${vmId}`);
      logger.debug(`   Data type: ${typeof data}`);
      logger.debug(`   Is Buffer: ${Buffer.isBuffer(data)}`);
      logger.debug(`   Is ArrayBuffer: ${data instanceof ArrayBuffer}`);
      logger.debug(`   Length: ${data.length || data.byteLength || 'unknown'}`);
      ...
      try {
        ...
        if (Buffer.isBuffer(data) || data instanceof ArrayBuffer) {
          const buffer = Buffer.from(data);
          logger.debug(`📤 [WS→TCP] ${vmId}: Sending ${buffer.length} bytes to SPICE server`);
        ...
        // Handle text messages (control messages)
          ...
          try {
            ...
            const buffer = Buffer.from(data);
            logger.debug(`📤 [WS→TCP] ${vmId}: Sending ${buffer.length} bytes (text->binary) to SPICE server`);
            if (target.writable) {
              target.write(buffer);
            } else {
              log(`⚠️ TCP socket not writable, cannot send data`);
          ...
          const buffer = Buffer.from(data);
          logger.debug(`📤 [WS→TCP] ${vmId}: Sending ${buffer.length} bytes (unknown type) to SPICE server`);
          if (target.writable) {
            target.write(buffer);
          } else {
            log(`⚠️ TCP socket not writable, cannot send data`);
        ...
        logger.error(`❌ [WS→TCP] ${vmId}: Error details:`, error);
      }
    });

    // TCP -> WebSocket: Forward server data to client
    logger.debug(`📝 [Proxy Setup] Registering 'data' event handler for TCP connection`);
    target.on('data', (data) => {
      try {
        logger.debug(`📥 [TCP→WS] ${vmId}: Received ${data.length} bytes from SPICE server`);
        // Hex dump first 64 bytes for SPICE protocol debugging
        if (data.length > 0) {
        ...
        if (ws.readyState === WebSocket.OPEN) {
          logger.debug(`   → Forwarding to WebSocket client (state: OPEN)`);
          ws.send(data, { binary: true });
          this.updateActivity(connectionId);
        } else {
          logger.error(`   ❌ Cannot forward: WebSocket state is ${ws.readyState} (not OPEN)`);
        }
      } catch (error) {
        log(`Error forwarding TCP->WS: ${error.message}`);
        logger.error(`❌ [TCP→WS] ${vmId}: Error details:`, error);
      }
    });
  ...
  cleanupConnection(connectionId, vmId) {
    ...
    if (vmConns) {
      ...
      if (vmConns.size === 0) {
        ...
        logger.debug(`🧹 All connections closed for VM ${vmId}`);
      }
    }
  }
  ...
  handleControlMessage(message, ws, target, vmId) {
    switch (message.type) {
      ...
      case 'resize':
        logger.debug(`[${vmId}] Screen resize: ${message.width}x${message.height}`);
        break;

      case 'quality':
        logger.debug(`[${vmId}] Quality adjustment: ${message.quality}`);
        break;

      case 'clipboard':
        logger.debug(`[${vmId}] Clipboard sync: ${message.text?.substring(0, 50)}...`);
        break;

      default:
        logger.debug(`[${vmId}] Unknown control message: ${message.type}`);
    }
  }
  ...
  closeVMConnections(vmId) {
    ...
    if (!connIds || connIds.size === 0) {
      logger.debug(`No connections to close for VM ${vmId}`);
      return;
    }

    logger.debug(`🔌 Closing ${connIds.size} connection(s) for VM ${vmId}`);
   ...
   */
  async shutdown() {
    logger.debug('🛑 Shutting down WebsockifyProxy...');

    this.stopHeartbeat();

    // Close all connections
    for (const [connectionId, conn] of this.connections.entries()) {
      logger.debug(`Closing connection ${connectionId} for VM ${conn.vmId}`);
      if (conn.ws.readyState === WebSocket.OPEN) {
    ...
    logger.debug('✅ WebsockifyProxy shutdown complete');
  }
